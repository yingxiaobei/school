  ...
  var CLODOP = {
    ...
    Printers: {
      ...
      list: [
        ...
        },
        {
          ...
          PortName:
            'Microsoft.Office.OneNote_16001.14326.20458.0_x64__8wekyb3d8bbwe_microsoft.onenoteim_S-1-5-21-2490347615-438033763-2207943719-1001',
          Orientation: '1',
          PaperSize: '9',
          PaperLength: '2970',
          PaperWidth: '2100',
        ...
        },
        {
          ...
          DriverName: 'HP LaserJet Professional M1213nf MFP',
          PortName: '172.16.9.88',
          Orientation: '1',
          PaperSize: '9',
          PaperLength: '2970',
          PaperWidth: '2100',
        ...
        {
          name: 'HP LaserJet Professional M1210 MFP Series Fax',
          DriverName: 'HP LaserJet Professional M1210 MFP Series Fax',
          PortName: '172.16.9.88',
          Orientation: '1',
          PaperSize: '9',
          PaperLength: '2970',
    ...
    OpenWebSocket: function () {
      ...
      try {
        ...
        this.webskt = null;
        if (err.message.indexOf('SecurityError') > -1) this.SocketEnable = false;
            ...
            if (oType.toLowerCase() == 'weekindex') oValue = iDay;
              ...
              if (/(m+:)/i.test(oValue))
                oValue = oValue.replace(
                  RegExp.$1,
                  ('00' + iMinutes + ':').substr(('00' + iMinutes + ':').length - RegExp.$1.length),
                );
              if (/(M+)/i.test(oValue)) {
              ...
              if (/(H+)/i.test(oValue))
                oValue = oValue.replace(RegExp.$1, ('00' + iHour).substr(('00' + iHour).length - RegExp.$1.length));
              if (/(n+)/i.test(oValue))
                oValue = oValue.replace(
                  RegExp.$1,
                  ('00' + iMinutes).substr(('00' + iMinutes).length - RegExp.$1.length),
              ...
              if (/(s+)/i.test(oValue))
                oValue = oValue.replace(RegExp.$1, ('00' + iSecond).substr(('00' + iSecond).length - RegExp.$1.length));
            }
          }
          if (CLODOP.On_Return) {
            var selfFunc = CLODOP.On_Return;
    ...
    ADD_PRINT_PDF: function (top, left, width, height, strContent) {
      if (!this.blIslocal && strContent[1] == ':') alert('Unable to print or view remote service files');
    ...
    SET_PRINT_STYLEA: function (ItemNo, strKey, Value) {
      ...
      for (var vItemNO in this.ItemDatas) {
        ...
        if (
          ItemNo == vItemNO ||
          ItemNo == ItemName ||
          (typeof ItemNo === 'string' && typeof ItemName === 'string' && ItemNo.toUpperCase() == ItemName.toUpperCase())
        ) {
          ...
          if (PNameIndx == intNO || PNameIndx == this.Printers['list'][intNO].name) {
            ...
            for (var iPNO in this.Printers['list'][intNO][ListName]) {
              if (NameOrID == 'id' && this.Printers['list'][intNO][ListName][iPNO].id) {
                ...
                else strList = strList + Split + this.Printers['list'][intNO][ListName][iPNO].id;
              } else {
                if (strList === '') strList = this.Printers['list'][intNO][ListName][iPNO].name;
                else strList = strList + Split + this.Printers['list'][intNO][ListName][iPNO].name;
              }
            }
            return strList;
          }
            ...
            if (BridgeNameOrIndex == iBdgNO || BridgeNameOrIndex == BridgeID + ';' + BridgeName) {
              ...
              for (var iSsbPNO in this.Printers['list'][iDrvNO]['subdevlist'][iBdgNO].ssbprinter['list']) {
                ...
                if (strListName.toLowerCase() == 'id') strList = strList + iSsbPNO;
                  ...
                  strList =
                    strList + this.Printers['list'][iDrvNO]['subdevlist'][iBdgNO].ssbprinter['list'][iSsbPNO].name;
              }
              return strList;
            }
          }
            ...
            if (BridgeNameOrIndex == iBdgNO || BridgeNameOrIndex == BridgeID + ';' + BridgeName) {
              for (var iSsbPNO in this.Printers['list'][iDrvNO]['subdevlist'][iBdgNO].ssbprinter['list']) {
                if (
                  ...
                  SubPrinterNameOrIndex ==
                    this.Printers['list'][iDrvNO]['subdevlist'][iBdgNO].ssbprinter['list'][iSsbPNO].name
                ) {
                  var strList = '';
                  for (var iPageSizeNO in this.Printers['list'][iDrvNO]['subdevlist'][iBdgNO].ssbprinter['list'][
                    iSsbPNO
                  ]['pagelist']) {
                    if (iPageSizeNO != '0') strList = strList + strSplit;
                    strList =
                      strList +
                      this.Printers['list'][iDrvNO]['subdevlist'][iBdgNO].ssbprinter['list'][iSsbPNO]['pagelist'][
                        iPageSizeNO
                      ].name;
                  }
                  return strList;
    ...
    PopView: function (destView, strPURL, iWidth, iHigh) {
      try {
        if (destView && typeof destView === 'string' && destView.length > 0) {
          ...
          } else if (destView === '_self' || destView === '_top' || destView === '_parent') {
